...
- **视图(V)**
  - 主题(T)
    - 亮色主题(L) - `Ctrl+Shift+L`: 切换到亮色主题
    - 暗色主题(D) - `Ctrl+Shift+D`: 切换到暗色主题
  - 媒体库(1) - `Ctrl+1`: 切换到媒体库选项卡
  - 匹配识别(2) - `Ctrl+2`: 切换到匹配识别选项卡
  - 重命名规则(3) - `Ctrl+3`: 切换到重命名规则选项卡
  - 历史记录(4) - `Ctrl+4`: 切换到历史记录选项卡
  - 清空日志(C) - `Ctrl+L`: 清空日志面板
...
- **启用分批重命名**: 提升大批量文件处理性能
- **并行工作线程数**: 设置并行重命名的线程数（1-16）
- **相似度阈值**: 设置匹配相似度阈值（0.5-0.9）

### 外观选项卡

- **主题设置**:
  - **亮色主题**: 白色背景，深色文字，适合光线充足的环境
  - **暗色主题**: 深色背景，浅色文字，适合低光环境，减少眼睛疲劳
  - 提示：主题切换将在保存后立即生效
...
### Q7: 如何切换主题
**A**: 有两种方式:
1. 通过菜单: `视图 -> 主题` 选择亮色或暗色主题
2. 使用快捷键: `Ctrl+Shift+L` (亮色) 或 `Ctrl+Shift+D` (暗色)
3. 通过设置: `编辑 -> 设置 -> 外观` 选择主题并保存
